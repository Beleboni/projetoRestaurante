...
public class PedidoJDBC implements PedidoDAO {
	...
	public Pedido buscar(Integer id) {
		...
		try {
			...
			while (rs.next()) {
				...
				pedido.setDataPedido(LocalDate.parse((rs
						.getString("dataPedido"))));
	...
	public List<Pedido> todos() {
		...
		try {
			...
			while (rs.next()) {
				...
				pedido.setCodigo(rs.getInt("codigoPedido"));
				pedido.setMesa(rs.getString("mesa"));
				pedido.setDataPedido(LocalDate.parse(rs.getDate("dataPedido")
						.toString()));
				pedido.setStatus(StatusPedido.get(rs.getString("status")));
				pedido.setTotal(rs.getDouble("total"));
				pedidos.add(pedido);
	...
	@Override
	public boolean isOcupada(Integer mesa) {
		String sql = "select p.codigoPedido as codigo from pedido p where status <> 'CONCLUIDO' and mesa = ?";
		try {
			PreparedStatement pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, mesa);
