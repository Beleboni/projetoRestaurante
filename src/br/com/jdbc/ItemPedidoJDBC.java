...
public class ItemPedidoJDBC implements ItemPedidoDAO {
	...
	public void inserir(ItemPedido itemPedido) {
		String sql =  "insert into itempedido (codigoItemPedido, codigoProduto, codigoPedido)" + "values (?,?,?)";
		try {
			PreparedStatement pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, itemPedido.getCodigo());
			pstmt.setInt(2, itemPedido.getProduto().getCodigo());
			pstmt.setInt(3, itemPedido.getPedido().getCodigo());
			pstmt.executeUpdate();	
